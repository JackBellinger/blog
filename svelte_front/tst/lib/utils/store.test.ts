...
describe('Filtering the project store by each title', () => {
	...
	it('should produce only one filtered item', async () => {
		projectStores.items.load().then(projects => projects.forEach(
			async project => {
				projectStores.filter.set(
					{searchTerm: project.title, selectedTags: new Set()}
				)
				await projectStores.filteredItems.load().then(
					filteredItems => {
	...
	it('should produce only items with such tags', async () => {
		projectStores.items.load().then(projects => projects.forEach(
			async project => {
				...
				projectStores.filter.set(
					{searchTerm: "", selectedTags: new Set(project.tags)}
				);
				...
				await projectStores.filteredItems.load().then(
					filteredItems => {
						filteredItems.forEach(filteredProject => {
							// filter by tags shows all the posts which have at least one of the tags
							expect(project.tags.some(
								tag => filteredProject.tags.includes(tag)
							)).toBeTruthy()
						})
						expect(filteredItems).toContain(project)
				})
